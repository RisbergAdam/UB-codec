...
void Main()
{
    ...
    var accBuffer = PlanarImage.FromBitmap(accFrame, 2);
    ...
    for (var i = 2; i < 2; i++)
    {
        TimeExec("iteration", () =>
        {
            ...
            var currBuffer = PlanarImage.FromBitmap(currFrame, 2);
            var encoded = TimeExec("encode", () => encoder.EncodeFrame(accBuffer, currBuffer, i));
            var frameRecBuffer = PlanarImage.FromSize(currBuffer.Width, currBuffer.Height, 2);
            TimeExec("decode", () =>
            {
