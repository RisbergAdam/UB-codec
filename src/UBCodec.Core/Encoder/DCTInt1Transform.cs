...
public class DctInt1Transform : ITransform
{
    ...
    private long[,] _workmem2 = new long[8, 8];
    ...
    public void TransformInverse(int blockSize, int[,] input, byte[,] output)
    {
        ...
        for (var xb = 0; xb < subBlocks; xb++)
        {
            ...
            for (var i = 0; i < 8; i++)
            {
                _workmem2[x, y] += _workmem[i, y] * M[x, i];
            }

            long norm = (long)QFactor * QFactor;
            long round = norm / 2;
            for (var y = 0; y < 8; y++)
            for (var x = 0; x < 8; x++)
            {
                var v = _workmem2[x, y];
                v = v >= 0 ? (v + round) / norm : -((-v + round) / norm);
                output[x + 8 * xb, y + 8 * yb] = (byte)Math.Clamp(v + 127, 0, 255);
            }
        }
    }
